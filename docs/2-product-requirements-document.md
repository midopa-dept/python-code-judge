...
> 업데이트(군번 제거): 군번은 더 이상 수집하지 않습니다. 회원가입/계정 관련 입력값은 ID, 이름, 비밀번호만 요구하며, 군번 필드는 사용하지 않습니다.
...
- **설명**: 학생이 군번, ID, 이름, 비밀번호로 회원가입
- **입력**: 군번(영문/숫자/하이픈 5-20자), 로그인 ID(4-20자), 이름(2-50자), 비밀번호(8-100자)
- **출력**: 회원가입 성공/실패 메시지
- **비즈니스 로직**: 군번과 ID 중복 검증, 비밀번호 bcrypt 암호화
- **예외**: 중복 군번/ID 시 에러 반환
...
- **설명**: 비밀번호 재설정 기능
- **입력**: 군번(영문/숫자/하이픈), 이름 (찾기), 기존/새 비밀번호 (변경)
- **출력**: 성공/실패 메시지
...
| 데이터 보호    | 개인정보 최소화 | 군번/이름만 수집, 이메일 선택             |
