...
**배경**: 조병장이 같은 문제를 연속으로 여러 번 제출하려고 합니다.

**사용 흐름**:

1. **제출 쿨타임 (5초 제한)**
   - 14:00:00 - 첫 번째 제출 → AC
   - 14:00:02 - 다시 제출 시도 (오타 발견)
   - ⚠️ 에러 메시지:
     ```
     제출 실패: 5초 이내 동일 문제 재제출이 불가능합니다.
     3초 후 다시 시도하세요.
     ```
   - 14:00:06 - 제출 성공
...
2. **동시 제출 제한 (최대 3개)**
   ...
   - ⚠️ 에러 메시지:
     ```
     제출 실패: 최대 3개 문제까지 동시 제출 가능합니다.
     채점이 완료될 때까지 기다려주세요.
     ```
   - 대기 후 제출 가능

3. **채점 큐 초과 (전체 500개 제한)**
   - 시스템 전체 제출이 폭주하는 상황
   - 학생이 제출 시도
   - ⚠️ 에러 메시지 (HTTP 429):
     ```
...
| 10. 제출 제한 상황            | 학생   | 변동      | 쿨타임, 동시 제출 제한, 큐 관리              | 시스템 제약 이해               |
