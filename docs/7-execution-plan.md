...
- [x] Express 서버 기본 설정
  ...
  - 의존성: 없음
  - **완료**: backend/src/app.js, backend/src/server.js 구현 완료

- [x] 에러 처리 미들웨어 구현

  - 완료 조건: AppError, ValidationError 등 커스텀 에러 클래스 정의
  - 의존성: 없음
  - **완료**: backend/src/shared/errors/AppError.js (6개 에러 클래스)

- [x] 로거 설정 (winston)

  - 완료 조건: 요청/응답 로깅, 에러 로깅 동작
  - 의존성: 없음
  - **완료**: backend/src/shared/utils/logger.js (파일 로깅 + 콘솔 출력)

- [x] CORS, 보안 헤더 설정 (helmet)
  - 완료 조건: 프론트엔드에서 API 호출 가능
  - 의존성: 없음
  - **완료**: backend/src/shared/middleware/security.js (Helmet + Rate Limiting)

##### 2. 인증 모듈 (auth/)

- [x] JWT 토큰 생성/검증 유틸리티 작성

  - 완료 조건: generateToken, verifyToken 함수 구현
  - 의존성: JWT_SECRET 환경 변수
  - **완료**: backend/src/modules/auth/utils/jwt.js

- [x] 인증 미들웨어 구현

  - 완료 조건: authMiddleware, requireRole 미들웨어 동작
  - 의존성: JWT 유틸리티
  - **완료**: backend/src/modules/auth/middleware/authMiddleware.js (RBAC 포함)

- [x] 학생 회원가입 API

  - 완료 조건: POST /api/auth/signup (학생)
  - 의존성: students 테이블, bcrypt
  - **완료**: POST /api/auth/signup 구현 및 테스트 완료

- [x] 로그인 API (학생/관리자 통합)

  - 완료 조건: POST /api/auth/login
  - 의존성: JWT 유틸리티
  - **완료**: POST /api/auth/login 구현 및 테스트 완료

- [x] 비밀번호 찾기/변경 API
  - 완료 조건: POST /api/auth/reset-password, PUT /api/auth/change-password
  - 의존성: 인증 미들웨어
  - **완료**: 2개 API 모두 구현 및 테스트 완료

##### 3. 사용자 관리 모듈 (users/)

- [x] 학생 목록 조회 API (관리자 전용)

  - 완료 조건: GET /api/users/students
  - 의존성: RBAC 미들웨어
  - **완료**: 페이지네이션, 검색, 필터링 기능 포함

- [x] 학생 상세 조회 API
  - 완료 조건: GET /api/users/students/:id
  - 의존성: 인증 미들웨어
  - **완료**: 본인/관리자 권한 분리, 통계 정보 포함
...
- [x] 문제 수정/삭제 API
  ...
  - 의존성: 문제 등록 API
  - **완료**: 부분 업데이트, cascade 삭제 지원

- [x] 카테고리별 문제 조회 API

  - 완료 조건: GET /api/problems?category=조건문
  - 의존성: problems 테이블
  - **완료**: 카테고리 목록 API (GET /api/categories) 추가 구현

- [x] 문제 상세 조회 API

  - 완료 조건: GET /api/problems/:id (공개 테스트 케이스 포함)
  - 의존성: test_cases 테이블
  - **완료**: 관리자/학생 권한별 테스트 케이스 필터링
...
- [x] 세션 학생/문제 할당 API
  ...
  - **완료**: 트랜잭션 처리, 중복 방지 (ON CONFLICT)

- [x] 세션 시작/종료 API

  - 완료 조건: PUT /api/sessions/:id/status
  - 의존성: education_sessions 테이블
  - **완료**: 상태 검증 (scheduled/active/ended/cancelled)

- [x] 세션 초기화 API (제출 이력 삭제)

  - 완료 조건: DELETE /api/sessions/:id/reset
  - 의존성: submissions, scoreboards 테이블
  - **완료**: 트랜잭션으로 제출/스코어보드 삭제
...
- [x] 감사 로그 기록 미들웨어
  ...
  - 의존성: audit_logs 테이블
  - **완료**: logAction 미들웨어를 주요 API에 적용 (auth, problems, sessions)

- [x] 감사 로그 조회 API (최고관리자 전용)
  - 완료 조건: GET /api/audit-logs
  - 의존성: audit_logs 테이블
  - **완료**: 페이지네이션, 필터링 (userId, actionType, startDate, endDate) 지원

**병렬 수행 완료**: 문제, 세션, 감사 로그 모듈을 병렬로 개발하여 효율성 향상
**통합 테스트**: 23개 시나리오 테스트 모두 성공 (100%)
